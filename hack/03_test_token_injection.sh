...
kubectl rollout status deployment/spi-vault-operator  -n spi
kubectl rollout status deployment/spi-controller-manager  -n spi
...
if [ $INJECTED_VALUE = "github_token_here" ]; then
  ...
  echo "vault events"
  kubectl get events --sort-by=.metadata.creationTimestamp -n spi
  echo "spi-system events"
  kubectl get events --sort-by=.metadata.creationTimestamp -n spi
  echo "operator logs"
  kubectl logs -n spi deployment/spi-controller-manager -c manager
  echo "SPI API logs"
  kubectl logs -n spi deployment/service-provider-integration-api -c service-provider-integration-api
  exit 1
fi
